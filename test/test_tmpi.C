...
void test_tmpi(int argc, char *argv[]) {
  ...
  optparse.add_options()("c,ncollectors", "number of collecting ranks to run",
                         cxxopts::value<Int_t>(N_collectors))(
      "r,syncrate",
      "number of events to generate per worker rank before sending to "
      "collector rank",
      cxxopts::value<Int_t>(sync_rate))(
      "n,evt_per_rank", "total events to generate per worker rank",
      cxxopts::value<Int_t>(events_per_rank))(
      "s,sleep_mean", "seconds to sleep, gaussian mean",
      cxxopts::value<Int_t>(sleep_mean))("t,sleep_sigma",
                                         "seconds to sleep, gaussian sigma",
                                         cxxopts::value<Int_t>(sleep_sigma))(
